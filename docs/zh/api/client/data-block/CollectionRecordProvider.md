...
用于提供 [CollectionRecord](./CollectionRecord) 实例。
...
参数的具体说明参见 [CollectionRecord](./CollectionRecord)。

需要说明的是 `record` 和 `parentRecord` 即可以是普通的对象，也可以是 [CollectionRecord](./CollectionRecord) 实例，但最终会转为 `CollectionRecord` 实例，并通过 context 传递给子组件。
