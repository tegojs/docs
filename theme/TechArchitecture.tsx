...
export const TechArchitecture = () => {
    ...
    const content = isEnglish
        ? {
            ...
            core:
                'Core Layer: Provides standardized ORM interfaces and transaction management, supporting cross-database consistency.',
            plugin:
        ...
        : {
            ...
            core:
                '提供标准化 ORM 接口与事务管理机制，支持跨数据库事务一致性。',
            plugin:
        ...
        >
            <div className="max-w-[1200px] mx-auto px-5 grid place-items-center gap-5">
                ...
                <div
                    ...
                    `}
                >
                    <strong className={`${isDark ? 'text-white/80' : 'text-[#000000dd]'}`}>{isEnglish ? 'Module Layer:' : '模块层：'}</strong> {content.module}
                </div>
                <div
                    className={`
                    ...
                    `}
                >
                    <strong className={`${isDark ? 'text-white/80' : 'text-[#000000dd]'}`}>{isEnglish ? 'Core Layer:' : '内核层：'}</strong> {content.core}
                </div>
                <div
                    className={`
                    ...
                    `}
                >
                    <strong className={`${isDark ? 'text-white/80' : 'text-[#000000dd]'}`}>{isEnglish ? 'Plugin Layer:' : '插件层：'}</strong> {content.plugin}
                </div>
            </div>
            <svg
                viewBox="0 0 100 20"
