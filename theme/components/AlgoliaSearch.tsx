...
const getTranslations = (isEnglish: boolean, hasAI: boolean) => ({
  ...
  modal: {
    ...
    startScreen: {
      ...
      saveRecentSearchButtonTitle: isEnglish
        ? 'Save this search'
        : '保存到最近搜索',
    ...
    const timer = setTimeout(() => {
      ...
      if (originalSearchButton) {
        ...
        if (parent) {
          ...
          // 确保搜索框在正确的 flex 容器中
          const flexContainer = parent.closest(
            '.rp-flex.sm\\:rp-flex-1, [class*="flex"]',
          );
          if (flexContainer && !flexContainer.contains(searchContainer)) {
            flexContainer.insertBefore(
              searchContainer,
              flexContainer.firstChild,
            );
          }
        }
      } else {
        ...
        if (navContainer) {
          const rightNav = navContainer.querySelector(
            '.rightNav_a2fea',
          ) as HTMLElement | null;
      ...
      const initializeSearch = (useAI: boolean) => {
        ...
        const existingButton =
          searchContainer.querySelector('.DocSearch-Button');
        if (existingButton) {
      ...
      if (!(window as any).__docsearch_fetch_intercepted) {
        ...
        window.fetch = async (...args) => {
          try {
            ...
            if (
              response.status === 401 &&
              (args[0]?.toString().includes('algolia') ||
                args[0]?.toString().includes('docsearch'))
            ) {
      ...
      return () => {
        ...
        window.removeEventListener(
          'docsearch-ai-error',
          handleAIError as EventListener,
        );
      };
    }, 200);
