...
} = require('../tools/code-block-utils');
...
if (!TASK_ID) {
  ...
  console.error(
    chalk.gray('用法:'),
    'node 1-merge-guides.js <taskId> [--strict]',
  );
  process.exit(1);
}
...
function main() {
  ...
  if (!fs.existsSync(GUIDES_DIR)) {
    console.error(
      `  ${chalk.red('❌ 错误:')} 源目录不存在: ${chalk.magenta(path.relative(ROOT_DIR, GUIDES_DIR))}`,
    );
  ...
  if (fs.existsSync(headerPath)) {
    ...
    console.warn(
      `  ${chalk.yellow('⚠️  警告:')} 未找到 ${chalk.magenta('scripts/merge-md/assets/header.md')}，使用默认标题`,
    );
    const now = new Date();
  ...
  fs.writeFileSync(
    MDX_PROCESSED_LOG,
    JSON.stringify(mdxProcessed, null, 2),
    'utf-8',
  );
  fs.writeFileSync(
    RELATIVE_LINKS_LOG,
    JSON.stringify(relativeLinksProcessed, null, 2),
    'utf-8',
  );
  fs.writeFileSync(
    RELATIVE_IMAGES_LOG,
    JSON.stringify(relativeImagesProcessed, null, 2),
    'utf-8',
  );
  fs.writeFileSync(
    MISSING_META_LOG,
    JSON.stringify(missingMetaFiles, null, 2),
    'utf-8',
  );
  fs.writeFileSync(
    HEADING_OVERFLOW_LOG,
    JSON.stringify(headingOverflows, null, 2),
    'utf-8',
  );

  // 输出统计
  console.log(
    `  ${chalk.green('✓')} 处理 ${chalk.cyan(stats.totalFiles)} 个文件`,
  );

  if (stats.mdxFiles > 0) {
    console.log(
      `  ${chalk.green('✓')} 处理 ${chalk.cyan(stats.mdxFiles)} 个 MDX 文件 ${chalk.dim('→ 详见')} ${chalk.magenta(path.relative(ROOT_DIR, MDX_PROCESSED_LOG))}`,
    );
  }

  if (stats.admonitions > 0) {
    console.log(
      `  ${chalk.green('✓')} 转换 ${chalk.cyan(stats.admonitions)} 个提示框为引用块`,
    );
  }

  if (stats.skippedCount > 0) {
    console.log(
      `  ${chalk.yellow('⚠️')}  跳过 ${chalk.cyan(stats.skippedCount)} 个文件 ${chalk.dim('→ 详见')} ${chalk.magenta(path.relative(ROOT_DIR, SKIPPED_FILES_LOG))}`,
    );
  }

  if (stats.relativeLinks > 0) {
    console.log(
      `  ${chalk.green('✓')} 转换 ${chalk.cyan(stats.relativeLinks)} 个相对路径链接 ${chalk.dim('→ 详见')} ${chalk.magenta(path.relative(ROOT_DIR, RELATIVE_LINKS_LOG))}`,
    );
  }

  if (stats.relativeImages > 0) {
    console.log(
      `  ${chalk.green('✓')} 转换 ${chalk.cyan(stats.relativeImages)} 个相对路径图片 ${chalk.dim('→ 详见')} ${chalk.magenta(path.relative(ROOT_DIR, RELATIVE_IMAGES_LOG))}`,
    );
  }
  ...
  if (missingMetaFiles.length > 0) {
    console.log(
      `  ${chalk.yellow('⚠️')}  缺失 _meta.json: ${chalk.cyan(missingMetaFiles.length)} 个 ${chalk.dim('→ 详见')} ${chalk.magenta(path.relative(ROOT_DIR, MISSING_META_LOG))}`,
    );
    ...
    if (missingMetaFiles.length > displayCount) {
      console.log(
        `     ${chalk.dim('... 以及 ' + (missingMetaFiles.length - displayCount) + ' 个其他文件')}`,
      );
    }
  }
  ...
  if (headingOverflows.length > 0) {
    console.log(
      `  ${chalk.yellow('⚠️')}  深层标题 (H9+): ${chalk.cyan(headingOverflows.length)} 个 ${chalk.dim('→ 详见')} ${chalk.magenta(path.relative(ROOT_DIR, HEADING_OVERFLOW_LOG))}`,
    );
    ...
    for (let i = 0; i < displayCount; i++) {
      ...
      console.log(
        `     ${chalk.dim(`  · H${item.targetLevel} "${item.title.substring(0, 40)}${item.title.length > 40 ? '...' : ''}"`)}`,
      );
    }
    if (headingOverflows.length > displayCount) {
      console.log(
        `     ${chalk.dim('  ... 以及 ' + (headingOverflows.length - displayCount) + ' 个其他标题')}`,
      );
    }
  }
  ...
  console.log(
    `  ${chalk.green('✓')} 输出: ${chalk.magenta(path.relative(ROOT_DIR, OUTPUT_FILE))} ${chalk.dim('(' + fileSizeMB + ' MB)')}`,
  );
}
...
function processDirectory(dirPath, depth) {
  ...
  if (!fs.existsSync(metaPath)) {
    // 记录缺失的 _meta.json 文件
    missingMetaFiles.push(
      path.relative(ROOT_DIR, metaPath).replace(/\\/g, '/'),
    );
    return result;
  }
  ...
  for (const item of meta) {
    if (typeof item === 'object' && item.type === 'dir') {
      ...
      processedInMeta.add(
        path.relative(GUIDES_DIR, filePath).replace(/\\/g, '/'),
      );
      ...
      if (stats.totalFiles % 10 === 0) {
        process.stdout.write(
          `\r     ${chalk.dim(`处理中... ${stats.totalFiles} 个文件`)}`,
        );
      }
      ...
      if (fileStat.size > MAX_FILE_SIZE) {
        ...
        console.warn(
          `  ${chalk.yellow('⚠️  警告:')} 文件过大 ${chalk.magenta(path.relative(ROOT_DIR, filePath))} (${sizeMB} MB)`,
        );
        if (STRICT_MODE) {
      ...
      try {
        ...
        console.warn(
          `  ${chalk.yellow('⚠️  警告:')} 无法读取文件 ${chalk.magenta(path.relative(ROOT_DIR, filePath))}: ${error.message}`,
        );
        skippedFiles.push({
          reason: 'read_error',
          path: path.relative(ROOT_DIR, filePath).replace(/\\/g, '/'),
...
function processMDX(content, filePath) {
  ...
  content = replaceOutsideCodeBlocks(
    content,
    codeBlocks,
    /\{\/\*[\s\S]*?\*\/\}/g,
    '',
...
function processSpecialSyntax(content, filePath) {
  ...
  content = content.replace(
    /:::\s*(\w+)(?:\{title=([^}]+)\}|\s+([^\n]+))?\s*\n([\s\S]*?):::/g,
    (match, type, titleInBraces, titleInline, innerContent) => {
      stats.admonitions++;
      ...
      const displayTitle =
        titleInBraces || (titleInline ? titleInline.trim() : typeInfo.label);
...
function scanUnprocessedFiles(dirPath) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // 检查是否已处理
      if (
        !processedInMeta.has(relativePath) &&
        !processedInMeta.has(relativePath.replace(/\\/g, '/'))
      ) {
        // 未在 meta 中的文件
        skippedFiles.push({
          reason: 'not_in_meta',
